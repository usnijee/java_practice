...
public class Practice2 {
    public static ArrayList<ArrayList<String>> solution(String[] strs) {
        ...
        for (String s : strs) {
            ...
            //sort
            sort(cArr);
            String key = String.valueOf(cArr); // 정렬된 cArr을 다시 String으로 변환

            if (!map.containsKey(key)) {
                /**
                 * 초기 key 맞는 빈 저장공간 put 하기
                 */
                map.put(key, new ArrayList<>()); // 정렬된 cArr을 key로 하는 해시맵 구성
            }

            map.get(key).add(s); // 해당 key
